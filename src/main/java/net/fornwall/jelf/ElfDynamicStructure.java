...
public class ElfDynamicStructure {
	...
	ElfDynamicStructure(final ElfParser parser, long offset, int size) throws IOException {
		...
		final MemoizedObject<ElfHashTable> hashTable;
		if (hashOffset > 0) {
			hashTable = new MemoizedObject<ElfHashTable>() {
				@Override
				protected ElfHashTable computeValue() throws ElfException, IOException {
					return new ElfHashTable(parser, parser.virtualMemoryAddrToFileOffset(hashOffset), -1);
				}
			};
		} else {
			throw new IllegalStateException(".dynamic NO hash entry");
		}
